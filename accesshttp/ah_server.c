...
static int init_methods_handler(void)
{
    ...
    ERR_RET_LN(add_handler("balance.deposit", matchengine, CMD_PLEDGE_DEPOSIT));
#endif
    ...
    ERR_RET_LN(add_handler("market.update", matchengine, CMD_MARKET_UPDATE));
    ERR_RET_LN(add_handler("asset.update", matchengine, CMD_ASSET_UPDATE));
    ERR_RET_LN(add_handler("market.sync", marketprice, CMD_MARKET_SYNC));
    ERR_RET_LN(add_handler("market.market", marketprice, CMD_MARKET_MARKET));
    ERR_RET_LN(add_handler("market.depth", matchengine, CMD_MARKET_DEPTH));
    ERR_RET_LN(add_handler("order.user_history", readhistory, CMD_USER_ORDER_HISTORY));
