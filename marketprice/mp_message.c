...
static int init_market(void)
{
    ...
    settings.market_num = json_array_size(r);
    ...
    for (size_t i = 0; i < json_array_size(r); ++i) {
        ...
        settings.markets[i].name = sdsnewlen(name, strlen(name)); 
        settings.markets[i].money = sdsnewlen(money, strlen(money)); 
        settings.markets[i].stock = sdsnewlen(stock, strlen(stock)); 
    }
...
json_t *get_market_status_today_limit(const char *market)
{
    struct market_info *info = market_query(market);
    if (info == NULL)
        return NULL;
...
{
    struct market_info *info = market_query(market);
	log_debug("get_market_kline_day begin: %s", market);
    if (info == NULL)
	{
        log_debug("can't find market: %s", market);
        return NULL;
	}
	
	log_debug("get_market_kline_day has find market: %s", market);
    ...
    for (; start <= end; start += interval) {
        ...
        for (int i = 0; i < step; ++i) {
            ...
            if (entry == NULL)
                continue;
            struct kline_info *item = entry->val;
			log_debug("get_market_kline_day entry:");
            if (kinfo == NULL)
                kinfo = kline_info_new(item->open);
            kline_info_merge(kinfo, item);
        }
	...
	if (count_result)
	{
		log_debug("get_market_kline_day result has: %s", json_dumps(result,2));
	}
	else
	{
		log_debug("get_market_kline_day result failed!");
