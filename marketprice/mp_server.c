...
static int on_cmd_market_sync(nw_ses *ses, rpc_pkg *pkg, json_t *params)
{
    if (json_array_size(params) != 0)
        return reply_error_invalid_argument(ses, pkg);

    if (init_message() < 0)
        return reply_error(ses, pkg, 20, "market sync failed");

    return reply_success(ses, pkg);
}

static int on_cmd_market_market(nw_ses *ses, rpc_pkg *pkg, json_t *params)
{
    if (json_array_size(params) != 0)
        return reply_error_invalid_argument(ses, pkg);
    ...
    for (size_t i = 0; i < settings.market_num; ++i) {
        ...
        if (strcmp(money, CNY) == 0 || strcmp(money, BTC) == 0 || strcmp(money, ETH) == 0 || strcmp(money, BCH) == 0) {
            json_t *entry = get_market_status_today_limit(name);
            if (entry == NULL) {
                sdsfree(cache_key);
                return reply_error_internal_error(ses, pkg);
            }
...
static void svr_on_recv_pkg(nw_ses *ses, rpc_pkg *pkg)
{
    ...
    case CMD_MARKET_STATUS_TODAY:
        ...
        break;
    case CMD_MARKET_SYNC:
        log_trace("from: %s cmd market sync, sequence: %u params: %s", nw_sock_human_addr(&ses->peer_addr), pkg->sequence, params_str);
        ret = on_cmd_market_sync(ses, pkg, params);
        if (ret < 0) {
            log_error("on_cmd_market_sync %s fail: %d", params_str, ret);
        }
        break;
    case CMD_MARKET_MARKET:
        log_trace("from: %s cmd market market, sequence: %u params: %s", nw_sock_human_addr(&ses->peer_addr), pkg->sequence, params_str);
        ret = on_cmd_market_market(ses, pkg, params);
        if (ret < 0) {
            log_error("on_cmd_market_market %s fail: %d", params_str, ret);
        }
        break;
    default:
