...
MYSQL_DATABASE_COIN=trade_coin
MYSQL_USER=exchange
...
if [ "$1" = 'mysqld' -a "`id -u`" = '0' ]; then
	...
	if [ ! -d "$DATADIR/mysql" ]; then
		...
		/bin/sh -c "${mysql}" <<-EOSQL
			...
			CREATE DATABASE $MYSQL_DATABASE_COIN;
			GRANT ALL ON $MYSQL_DATABASE_HISTORY.* TO '$MYSQL_USER'@'%' ;
			GRANT ALL ON $MYSQL_DATABASE_COIN.* TO '$MYSQL_USER'@'%' ;
		...
		mysql="mysql --protocol=socket -u${MYSQL_USER} -p${MYSQL_PASSWORD} -hlocalhost --socket=${SOCKET} $MYSQL_DATABASE_COIN"
		echo "${mysql} < /docker-entrypoint-initdb.d/create_trade_coin.sql";
		/bin/sh -c "${mysql}" < "/docker-entrypoint-initdb.d/create_trade_coin.sql";
