...
{
    sds sql = sdsempty();
    sql = sdscatprintf(sql, "SELECT buy_coin_id, sell_coin_id, min_count from system_trade_type where status=1 and sell_coin_id<200");
    ...
    for (size_t i = 0; i < settings.market_num; ++i) {
        ...
        const char *min_count = row[2];
 
        mpd_t *amount = mpd_new(&mpd_ctx);
        mpd_set_string(amount, min_count, &mpd_ctx);
        settings.markets[i].min_amount = amount;
...
mpd_t *get_user_freeze_balance(uint32_t user_id, const char *asset, int prec)
{
    ...
    //    }else {
          //  mpd_sub(total, total, change, &mpd_ctx);
    //    }
   //log_stderr("row[0][%d]: %s", i, row[0]);
		mpd_del(change);
    }
    mysql_free_result(result);
	mysql_close(conn);
