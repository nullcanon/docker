...
static int on_cmd_balance_freeze(nw_ses *ses, rpc_pkg *pkg, json_t *params)
{
    ...
    int ret = pledge_user_balance(true, user_id, asset, "pledge", business_id, change, detail);
    mpd_del(change);
    if (ret == -1) {
        return reply_error(ses, pkg, 10, "repeat update");
    } else if (ret == -2) {
...
static int on_cmd_balance_deposit(nw_ses *ses, rpc_pkg *pkg, json_t *params)
{
    if (json_array_size(params) != 6)
        return reply_error_invalid_argument(ses, pkg);
...
static void svr_on_recv_pkg(nw_ses *ses, rpc_pkg *pkg)
{
    ...
    case CMD_PLEDGE_DEPOSIT:
        ...
        ret = on_cmd_balance_deposit(ses, pkg, params);
        if (ret < 0) {
            log_error("on_cmd_balance_deposit %s fail: %d", params_str, ret);
        }
        break;
#endif
    case CMD_ASSET_LIST:
