...
json_t *get_user_list()
{
    ...
    json_t *uid_list = json_array();
    for (size_t i = 0; i < 100; ++i) {
        sds sql = sdsempty();
        sql = sdscatprintf(sql, "SELECT `user_id` from `balance_history_%u` group by user_id", i);
        log_trace("exec sql: %s", sql);
        ...
        for (size_t i = 0; i < num_rows; ++i) {
            ...
            uint64_t user_id = strtoull(row[0], NULL, 0);
            json_array_append_new(uid_list, json_integer(user_id));
        }
