...
static int load_oper(json_t *detail)
{
	...
	log_stderr("load_oper load_update_balance ret: %d", ret);
	...
	//log_stderr("load_oper load_limit_order ret: %d", ret);
    } else if (strcmp(method, "market_order") == 0) {
        ret = load_market_order(params);
	//log_stderr("load_oper load_market_order ret: %d", ret);
    } else if (strcmp(method, "cancel_order") == 0) {
        ret = load_cancel_order(params);
	//log_stderr("load_oper load_cancel_order ret: %d", ret);
    } else {
        return -__LINE__;
    }
